...
#         height, width = image.shape[:2]
    ...
    def get_relevant_features(self, img_in):
        ...
        for (i, j) in [(50, 61), (52, 63), (67, 58), (66, 57), (65, 56)]:
            for (k, l) in [(49, 53), (33, 51), (27, 51), (33, 62)]:
        ...
        for (i, j) in [(49, 53), (59, 55), (60, 64), (48, 54)]:
            for (k, l) in [(27, 8), (38, 43), (4, 12), (27, 51)]:
                features.append(distance.euclidean(landmark_points[i], landmark_points[j])
                                / distance.euclidean(landmark_points[k], landmark_points[l]))
...
model_path = "Trained_models/Lips/male_lips_personalisation_xgb_v1.joblib"

# if not os.path.exists(model_path):
#     os.system("gsutil cp gs://ds-staging-bucket/3D-Hikemoji-pers/trained-models/male_lips_personalisation_xgb_v1.joblib " + model_path)
        ...
        if csv_file is not None:
            if gender == 'm':
                if curr_pred is None:
                    ...
                    curr_pred = curr_pred + [curr_pred[-1]]
